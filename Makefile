...
run:
		...
		echo "start running"
		docker run --rm -it -p 8000:8000/tcp --name ${PROJECT_NAME} --env-file .env ${PROJECT_NAME}:latest
...
serve:
		...
		-docker container rm ${PROJECT_NAME}
		echo "start running"
		docker run -d -p 8000:8000/tcp --name ${PROJECT_NAME} --env-file .env ${PROJECT_NAME}:latest
